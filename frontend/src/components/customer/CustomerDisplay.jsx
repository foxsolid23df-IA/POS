...
const CustomerDisplay = () => {
  ...
  useEffect(() => {
    ...
    const fetchCartSafe = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          console.log(
            "No se encontró información para la sesión activa:",
            sessionId,
          );
        }
      } catch (err) {
    ...
    const subSpecific = activeCartService.subscribeToCart(
      userId,
      sessionId,
      (newCart) => {
        if (newCart) {
  ...
  const shouldShowWelcome =
    !cart || (cart.cart_data?.length === 0 && status === "active");

  if (shouldShowWelcome) {
    return (
      <div className="customer-display-welcome">
        <div className="welcome-logo-container">
          <img src={logo} alt="NEXUM POS" className="welcome-brand-logo" />
        </div>
              ...
              {parseFloat(cart.amount_received) > 0 && (
                <>
                  <div className="info-row">
                    ...
                    <span className="info-value">
                      ${parseFloat(cart.amount_received).toFixed(2)}
                    </span>
                  </div>
                  <div className="info-row change-row">
                    <span className="info-label">Su Cambio</span>
                    <span className="info-value cambio-value">
                      ${parseFloat(cart.change_amount).toFixed(2)}
                    </span>
                  </div>
                </>
              )}
      ...
      <footer className="display-footer">
        <div className="footer-content">
          <div className="footer-brand">
            ...
            <span className="store-name-footer">NEXUM POS</span>
          </div>
