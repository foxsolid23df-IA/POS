        ...
        <>
            ...
            <header className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 z-[1001]">
                <button className="p-2 text-slate-500" onClick={toggleSidebar}>
                    <span className="material-icons-outlined text-[24px]">{isOpen ? 'close' : 'menu'}</span>
                </button>
                <div className="flex items-center gap-2 font-bold text-lg tracking-tight dark:text-white">
                    <span className="bg-primary text-white p-1 rounded">
                        <span className="material-icons-outlined block text-[18px]">point_of_sale</span>
                    </span>
                ...
                <div className="flex items-center gap-1">
                    ...
                    <button className="p-2 text-slate-500" onClick={lockScreen}>
                        <span className="material-icons-outlined text-[24px]">account_circle</span>
                    </button>
                </div>
            </header>
            ...
            {/* Sidebar */}
            <aside className={`
                fixed md:static inset-y-0 left-0 z-[1002] w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 
            ...
            `}>
                ...
                <div className="p-6 border-b border-slate-200 dark:border-slate-800">
                    <div className="flex items-center gap-3 font-bold text-xl tracking-tight dark:text-white">
                        <span className="bg-primary text-white p-2 rounded-lg shadow-lg shadow-black/10">
                            <span className="material-icons-outlined block text-[24px]">point_of_sale</span>
                        </span>
                        <div className="flex flex-col">
                            <span className="leading-none">{storeName || 'POS Store'}</span>
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Premium Retail</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="px-6 py-4 flex items-center gap-3">
                    ...
                    <div className="flex flex-col overflow-hidden">
                        <span className="text-sm font-bold text-primary dark:text-white truncate">{displayName}</span>
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{displayRole}</span>
                    </div>
                </div>
                ...
                <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
                    ...
                    <NavLink
                        ...
                        onClick={() => setIsOpen(false)}
                    >
                        <span className="material-icons-outlined text-[20px]">local_shipping</span>
                    ...
                    <NavLink
                        ...
                        onClick={() => setIsOpen(false)}
                    >
                        <span className="material-icons-outlined text-[20px]">currency_exchange</span>
                        <span className="text-sm font-bold">Dólares</span>
                    </NavLink>

                    <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800">
                        <button 
                            onClick={() => { setShowCashCut(true); setIsOpen(false); }} 
                        ...
                        >
                            <span className="material-icons-outlined text-[20px]">monetization_on</span>
                            <span className="text-sm font-bold">Corte de Caja</span>
                        </button>

                        <button 
                            onClick={() => { lockScreen(); setIsOpen(false); }} 
                ...
                <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-black/20">
                    ...
                    >
                        <div className="flex items-center gap-2">
                            <span className="material-icons-outlined text-[18px] group-hover:rotate-12 transition-transform">dark_mode</span>
