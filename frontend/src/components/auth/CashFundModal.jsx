...
export const CashFundModal = ({ staffName, staffId, onSessionCreated }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Swal.fire({
        title: "¡Caja Abierta!",
        html: `<p>Fondo inicial: <strong>${formatMoney(
          openingFund,
        )}</strong></p>`,
        icon: "success",
        timer: 2000,
    ...
    <div className="cash-fund-overlay">
      <div className="cash-fund-modal">
        ...
        <div className="quick-amounts">
          <button onClick={() => handleQuickAmount(0)} className="quick-btn">
            $0
          </button>
          <button onClick={() => handleQuickAmount(100)} className="quick-btn">
            $100
          </button>
          <button onClick={() => handleQuickAmount(500)} className="quick-btn">
            $500
          </button>
          <button onClick={() => handleQuickAmount(1000)} className="quick-btn">
            $1,000
          </button>
          <button onClick={() => handleQuickAmount(2000)} className="quick-btn">
            $2,000
          </button>
        </div>
        ...
        {/* Opción para todos: Ver sistema sin abrir caja (Útil para hacer Cierre de Día) */}
        <button
          className="cash-fund-skip-btn"
          onClick={() => {
            // Navegar a inventario para cerrar el modal y permitir otras acciones (como Cierre de Día)
