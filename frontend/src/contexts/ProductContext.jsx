...
export const ProductProvider = ({ children }) => {
  ...
  const loadProducts = useCallback(async (options = {}, retryCount = 0) => {
    ...
    try {
      console.log(
        `[ProductContext] Iniciando carga (Intento ${retryCount + 1})...`,
        { forceRefresh },
      );
      ...
      if (isMounted.current) {
        ...
        console.log(
          "[ProductContext] Productos cargados exitosamente:",
          safeProds.length,
        );
      ...
      console.error(
        `[ProductContext] Error cargando productos (Intento ${retryCount + 1}):`,
        err,
      );
      ...
      if (isMounted.current && retryCount < 2) {
        ...
        setTimeout(() => {
          if (isMounted.current) {
            ...
            loadProducts({ forceRefresh, silent }, retryCount + 1);
          }
  ...
  useEffect(() => {
    ...
    const subscription = productService.subscribeToProducts((payload) => {
      ...
      try {
        ...
        console.error(
          "[ProductContext] Error procesando evento realtime:",
          err,
        );
      }
    });
    ...
    return () => {
      console.log(
        "[ProductContext] Limpiando suscripción y marcando como desmontado",
      );
